...
export const getArticuloById = async (req, res) => {
	try {
		const [rows] = await pool.query("SELECT * FROM full_articulos WHERE id = ?", [
			req.params.id,
		]);
		if (rows.length === 0)
...
const verifyFiles = async (files) => {
	if (!files.archivo && !files.portada) throw new Error("Falta archivo y portada");
...
const deleteFiles = async (files) => {
	if (files.archivo) await unlink(`${FILES_DEST_DIR}${files.archivo[0].filename}`);
	if (files.portada) await unlink(`${FILES_DEST_DIR}${files.portada[0].filename}`);
};
...
export const patchEstadoArticulo = async (req, res) => {
	try {
		const { id_articulo, id_estado } = req.body;
		const [rows] = await pool.query(
			"UPDATE articulos SET id_estado = ? WHERE id_articulo = ?",
			[id_estado, id_articulo],
		);
